 ...
 * 卡片视图的目标分组列表
 *
 * 功能说明：将目标按父目标层级关系进行分组，便于在卡片视图中展示
 * 业务逻辑：
 * 1. 如果不是卡片视图，返回空数组
 * 2. 对于每个目标，计算其父目标路径：
 *    - 从当前目标的parentId开始
 *    - 递归向上查找父目标，最多追溯5层
 *    - 将父目标标题拼接成路径字符串（如"五年目标 > 年度目标"）
 *    - 如果没有父目标，归类为"独立目标"
 * 3. 根据活动筛选类型决定分组策略：
 *    - 未筛选或筛选"五年目标"时，所有目标归为"全部目标"
 *    - 其他类型按父目标路径分组
 * 4. 对分组进行排序：
 *    - "全部目标"排最前
 *    - "独立目标"排最后
 ...
 * 加载目标列表
 *
 * 功能说明：根据当前视图模式和筛选类型加载目标数据
 * 业务逻辑：
 * 1. 设置加载状态为true
 * 2. 根据视图模式调用不同接口：
 *    - 树形视图：调用goalApi.getTree()获取树形结构数据（包含父子关系）
 *    - 卡片视图：调用goalApi.list()获取扁平列表
 * 3. 同时加载全量目标列表（用于父级目标选择器）
 * 4. 加载统计信息（总数、进行中、已完成、平均进度）
 *
 * 使用场景：组件初始化、切换视图模式、切换筛选类型时调用
 *
 ...
 * 获取父目标标题
 *
 * 功能说明：根据父目标ID查找并返回父目标的标题
 * 业务逻辑：在allGoals列表中查找对应ID的目标并返回其title
 *
 * 使用场景：在目标卡片中显示"属于父目标: XXX"时调用
 *
 * @param parentId - 父目标的ID
 * @returns 父目标标题，如果未找到则返回"未知"
 */
 ...
 * 添加关键结果（KR）
 *
 * 功能说明：在目标表单中新增一个空的KR项
 * 业务逻辑：
 * 向keyResults数组push一个默认KR对象：
 * - title: 空字符串（待用户输入）
 * - targetValue: 100（默认目标值）
 * - currentValue: 0（默认当前值）
 * - unit: '%'（默认单位）
 *
 * 使用场景：用户点击"添加 KR"按钮时调用
 */
function addKr() {
 ...
 * 移除关键结果（KR）
 *
 * 功能说明：从目标表单中删除指定索引的KR项
 * 业务逻辑：使用splice方法从keyResults数组中移除对应项
 *
 * 使用场景：用户点击KR的删除图标时调用
 *
 * @param index - 要移除的KR在数组中的索引
 */
 ...
 * 功能说明：实时更新目标的进度百分比
 * 业务逻辑：
 * 1. 调用goalApi.updateProgress()提交新进度
 * 2. 后端会自动计算KR的平均进度
 *
 * 使用场景：用户拖动进度条滑块时调用
 *
 ...
 * 保存目标（新增或编辑）
 *
 * 功能说明：将用户填写的目标表单数据保存到后端
 * 业务逻辑：
 * 1. 表单验证
 * 2. 设置保存状态为true
 * 3. 根据editing状态判断是新增还是编辑：
 *    - 如果有editing值，调用goalApi.update()更新目标
 *    - 否则调用goalApi.create()创建新目标
 * 4. 成功后显示提示消息并关闭弹窗
 * 5. 重新加载目标列表
 *
 * 使用场景：用户在目标弹窗中点击"保存"按钮时调用
 *
 * @returns Promise<void>
 */
async function save() {
