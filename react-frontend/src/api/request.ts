 ...
 *    通过 Ant Design 的 message 组件统一展示错误提示。
 *
 * 【学习要点】
 * - Axios 实例的创建与配置
...
// message：Ant Design 的消息提示组件，用于在页面顶部显示操作反馈
import { message } from 'antd'


// useAppStore：Zustand 全局应用状态 Store，用于管理 Loading 状态和请求计数
...
request.interceptors.request.use(
  ...
  (config: InternalAxiosRequestConfig) => {
    ...
    // 注意：Token 是在用户登录成功后存储到 localStorage 的（见 stores/useUserStore.ts）
 ...
 * 例如：登录失败时可能需要自定义错误提示样式，而不是使用全局的 message。
 */
function handleBusinessError(errorMessage: string, config?: InternalAxiosRequestConfig) {
  // 如果请求配置中明确指定不显示错误提示，则直接返回
  if (config?.showError === false) return

  // 使用 Ant Design 的 message 组件弹出错误提示
...
function handleNetworkError(error: AxiosError) {
  ...
  if (error.response) {
    ...
    switch (status) {
      case 400:
        errorMsg = '请求参数错误'           // Bad Request：请求参数格式不正确
        break
      case 401:
        errorMsg = '登录已过期，请重新登录'  // Unauthorized：Token 过期或无效
        break
      case 403:
        errorMsg = '没有权限执行此操作'      // Forbidden：无权限访问该资源
        break
      case 404:
        errorMsg = '请求的资源不存在'        // Not Found：API 路径错误或资源已删除
        break
      case 500:
        errorMsg = '服务器错误，请稍后重试'  // Internal Server Error：后端代码异常
        break
      case 502:
        errorMsg = '网关错误'               // Bad Gateway：反向代理无法连接后端
        break
      case 503:
        errorMsg = '服务暂时不可用'          // Service Unavailable：服务过载或维护中
        break
      case 504:
        errorMsg = '请求超时'               // Gateway Timeout：网关等待后端响应超时
        break
      default:
        // 其他未明确处理的错误码，尝试使用后端返回的错误消息
        errorMsg = (error.response.data as any)?.message || `请求失败 (${status})`
    }
...
function handleAuthError() {
  ...
  // 跳转到登录页面
  // 在 React 中，我们不能直接使用 useNavigate，需要在组件中调用
  // 这里我们使用 window.location 进行重定向
...
// 导出 InternalAxiosRequestConfig 类型，供其他文件在需要时引用
